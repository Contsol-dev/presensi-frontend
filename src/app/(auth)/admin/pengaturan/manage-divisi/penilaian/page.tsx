...
export default function Penilaian() {
  ...
  const fetchPenilaianData = async () => {
    try {
      const response = await axios.get(
        ...
        }/admin/manage-penilaian/${localStorage.getItem("divisi_id")}`
      );
  ...
  const fetchNamaDivisi = async () => {
    try {
      const response = await axios.get(
        `${
          process.env.NEXT_PUBLIC_API_SERVER
        }/admin/manage-divisi/get/${localStorage.getItem("divisi_id")}`
      );
  ...
  const fetchKategoriPenilaian = async () => {
    try {
      const response = await axios.get(
        `${
          process.env.NEXT_PUBLIC_API_SERVER
        }/admin/manage-penilaian-kategori/${localStorage.getItem("divisi_id")}`
      );
  ...
  const handleDeleteSubcategory = async (id: any) => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_SERVER}/admin/manage-penilaian-subkategori/${id}`
      );
  ...
  const handleAddSubcategory = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_SERVER}/admin/manage-penilaian-subkategori`,
        subCNew
      );
  ...
  const handleDeleteCategory = async (id: any) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_SERVER}/admin/manage-penilaian-kategori/${division}/${id}`
      );
      console.log(response.data);
      fetchKategoriPenilaian();
      fetchPenilaianData();
  ...
  const handleAddCategory = async () => {
    try {
      console.log(CNew);
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_SERVER}/admin/manage-penilaian-kategori`,
        CNew
      );
      console.log("Response:", response.data);
      fetchKategoriPenilaian();
        ...
        <div className="bg-gray-200 w-full p-4 flex-col relative overflow-y-auto">
          ...
          <div className="flex items-center">
            ...
            <div className="ml-7 font-bold">
              Kategori Penilaian Divisi <span>{namaDivisi}</span>
            </div>
          </div>

          <div className="bg-black h-2 w-full relative">
            <div className="absolute top-2 left-0 p-6 text-white">
              Tambah Kategori Penilaian Divisi <span>{namaDivisi}</span>
            </div>
            <p className="absolute top-12 left-0 p-6 text-xs text-white">
              Digunakan untuk menentukan kategori penilaian kepada Peserta
              Magang
            </p>
          </div>
                ...
                <div className="modal-content">
                  ...
                  <div className="bg-gray-200 rounded-md p-4 mb-4">
                    {kategori.map((item, index) => (
                      <div
                        key={index}
                        className="w-full mt-4 text-box relative"
                      >
          ...
          <div className="bg-white p-4 mt-4 white-box relative">
            <button
              className="mt-4 bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 add-category"
              onClick={openModal}
            >
              + Tambah Kategori
            </button>
            <div className="flex flex-wrap mt-4">
              {penilaian.map((item, index) => (
                <div key={index} className="w-1/2 p-2">
                  ...
                  <div className="flex items-center mt-2">
                    <input
                      onChange={(e) => handleInputChange(e, item.kategori.id)}
                      type="text"
                      className="border-gray-300 bg-gray-100 border p-2 rounded-md"
                      style={{ width: "65%" }}
                    />
                    <button
                      ...
                      className="bg-red-600 text-white px-4 py-2 ml-2 rounded-md hover:bg-red-800"
                    >
                      Tambah
                    </button>
                  </div>
